...
| in_progress | Codex | Add version attribution to Frame.io comments and mirrored Notion feedback | `main.py`, Notion schema, `README.md`, `project_context.md`, `CHANGELOG.md`, `TASKS.md`, `HANDOFF.md` | Branch `feature/frameio-comment-version-attribution`; runtime now resolves comment versions through V4 `files/{file_id}` plus `version_stacks/{version_stack_id}/children`, staging validation passed for linked page `31839c2f-efe7-81dd-8bd3-ca760c9a7a63`, and `Last Frame Comment Version` persisted as `1`; a deeper temporary scan found no version stacks in the current Frame.io project, so observing `Version > 1` now requires uploading a new version over a linked asset or pointing the branch at another project with real version stacks |
