...
|----------|--------|-------|------|-------|-------|
| P1 | pending | Unassigned | Harden public webhook exposure for a public GitHub repo | `main.py`, `deploy.sh`, `README.md`, `project_context.md`, `BUGS.md`, `TASKS.md`, `HANDOFF.md` | Current deployment is `--allow-unauthenticated`, `/frameio-webhook` has no signature/secret validation, `/notion-webhook` has no caller verification, and `GET /` exposes `notion_db`; implement request authentication and reduce public metadata before the public repo becomes an attack guide |
