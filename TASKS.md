...
| Priority | Status | Owner | Task | Files | Notes |
|----------|--------|-------|------|-------|-------|
| P1 | pending | Unassigned | Harden public webhook exposure for a public GitHub repo | `main.py`, `deploy.sh`, `README.md`, `project_context.md`, `BUGS.md`, `TASKS.md`, `HANDOFF.md` | Factibility confirmed: Frame.io supports HMAC verification via `X-Frameio-Request-Timestamp` + `X-Frameio-Signature`, and Notion webhook actions support custom headers for a shared secret. Implement app-layer auth first behind flags; do not remove `--allow-unauthenticated` as a first step because both webhook providers need a public HTTP endpoint. Frame.io hardening depends on having the current signing secret or recreating the webhook if it was not stored. |
| P1 | pending | Unassigned | Investigate `BUG-006` for `Cambios Solicitados` -> `Changes requested` | `main.py`, `BUGS.md`, `HANDOFF.md` | `frameio_status: updated` is reported but the asset remains in `Needs Review`; do not use this status as a round signal until confirmed |
| P1 | pending | Unassigned | Expand the Frame.io webhook to include `comment.completed` / `comment.uncompleted` and refresh review signals from those events | Frame.io webhook config, `main.py`, `README.md`, `project_context.md`, `HANDOFF.md` | Official Frame.io docs confirm both webhook events exist; implement by subscribing the live webhook, mapping both events to `fio_get_comment_signals()`, and updating `Client Review Open` / open-resolved counts without changing native Notion discussion state |
| P2 | pending | Unassigned | Investigate why direct local Frame.io API calls return `403` while equivalent reads succeed inside the Cloud Function | Local auth flow, Frame.io permissions, Secret Manager tokens, staging diagnostics, `project_context.md`, `BUGS.md`, `TASKS.md`, `HANDOFF.md` | During validation of `feature/frameio-comment-version-attribution`, direct local calls using refreshed OAuth tokens kept returning `403`, while staging/live Cloud Function requests could read the same file and comment resources; isolate whether the difference comes from token audience/scope, account context, secret freshness, or tenant-level policy before relying on local Frame.io diagnostics again |
| P2 | pending | Unassigned | Validate whether Frame.io exposes a supported API write path to complete or reopen comments | Frame.io official docs, prototype script, `project_context.md`, `TASKS.md`, `HANDOFF.md` | Webhooks and comment reads are documented, but the public docs do not yet establish a stable completion-status mutation contract; do not implement `Notion -> Frame.io` comment completion until this is confirmed against official docs and a live prototype |
| P3 | pending | Unassigned | Design the structured Notion-side UX for "resolve in Frame.io" without relying on native Notion comment resolution | Notion schema, `README.md`, `project_context.md`, `TASKS.md`, `HANDOFF.md` | Native Notion comment resolve/reopen is not observable or controllable via public API; if users need an action from Notion, it must be modeled with properties, buttons, or an auxiliary feedback database keyed by `Frame Comment ID` |

## Completed Tasks

Historial reciente de tareas cerradas.
...
| done | Codex | Implement workflow-backed review rounds for tasks without Frame.io | `main.py`, `README.md`, `project_context.md`, `CHANGELOG.md`, `TASKS.md`, `HANDOFF.md` | Merged into `main` and deployed to production; validated in staging with page `31c39c2f-efe7-811a-9b6e-f40938fd0946` before rollout |
...
| done | Claude | Create annotated git tags for all released versions (`v2.0.0` to `v2.3.2`) and push to GitHub | git tags, `HANDOFF.md`, `project_context.md`, `CHANGELOG.md`, `TASKS.md` | Tags mapped to correct commits and pushed; future releases should create tags as part of the publish process |
