...
| in_progress | Codex | Design the Notion-only review-round flow and prepare a feature branch for implementation | `README.md`, `project_context.md`, `CHANGELOG.md`, `TASKS.md`, `HANDOFF.md` | Planned architecture: keep Frame.io-backed rounds in backend, add workflow-backed rounds for tasks without `Frame Asset ID`, and unify reporting through a final computed round field; implementation will start in `feature/notion-workflow-change-rounds` |
