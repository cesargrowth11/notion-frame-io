...
| in_progress | Codex | Add version attribution to Frame.io comments and mirrored Notion feedback | `main.py`, Notion schema, `README.md`, `project_context.md`, `CHANGELOG.md`, `TASKS.md`, `HANDOFF.md` | Branch `feature/frameio-comment-version-attribution`; code now resolves comment version from `comment.file_id` + version stack order, writes `Last Frame Comment Version` when the schema allows it, and adds `Version: N` to mirrored comments; pending verification and final schema decision |
